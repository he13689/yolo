...
# Image sizes
gs = max(int(model.stride.max()), 32)  # 网格大小
                ...
                with torch.no_grad():
                    ...
                    for i in range(imgs.shape[0]):  # 改为imgs.shape[0]， 因为这里loader没启用drop last，所以不能用batch size作为循环的次数
                        ...
                        target = [x for x in targets if
                                  x[0] == i]  # tensor([   0.0000,   23.0000,  585.5040,  459.5063,    0.1703,    0.5640])]
                        ...
                        save_box_img(image, target, image.shape[2], image.shape[1], prefix=f + '_gt.jpg', names=dataparams.get('names'))
                        save_box_img(image, pr, image.shape[2], image.shape[1], prefix=f + '_pred.jpg', names=dataparams.get('names'))
