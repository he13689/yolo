...
from yolov5_official.utils import xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str, xyxy2xywhn, Albumentations, mixup, copy_paste, letterbox
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # rect和mosaic只能用一个  启用数据增强后 会随机有一定几率的使用 mosaic
        self.mosaic = self.augment and not self.rect  # 将四张图片经过改变后接在一起
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        # nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found, empty, duplicate
        ...
        # x['msgs'] = msgs # 兩個dataset模块的差异就在于一个msg
        try:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w) = load_image(self, index)  # img, hw_original, hw_resize
            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            # 对图像进行自适应黑边填充 保证最小填充面积 影响网络训练
            ...
            if self.augment:  # 使用augment 图像增强开始
                # 是否使用mosaic 增强！
